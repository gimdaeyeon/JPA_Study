...
@Transactional
//Junit5에서 지원하는 어노테이션이며 테스트를 진행할 때 필요한 확장기능을 설정하는 어노테이션인다.
@ExtendWith(MockitoExtension.class)
//    Mock객체를 사용하는 이유
//    서비스를 테스트하고자 하는데 Repository까지 테스트를 한다면 불필요한 테스트가 진행되는 것이다.
//    그러므로 실제 Repository 객체를 만들어 컨테이너에 올릴 필요가 없다.
//    테스트하고자 하는 서비스객체만 만들고 나머지는 Mock(가짜)객체를 사용하여 테스트를 진행해야 단위 테스트의 목적에 맞다.
...
//        userRepository는 가짜 객체이기 때문에 각 메소드의 껍데기만 존재하고 내무에 실제 코드는 없다.
//        그렇기 때문에 해당 메소드를 호출하면 아무일도 일어나지 않는다.
//        개발자는 테스트를 위해 doReturn(), when()등을 이용하여 메소드 테스트 목적에 맞게 재정의할 수 있다.(스터빙)

//        given (준비)
//        레파지토리의 save()라는 메소드를 실행시킬 때 아무 값이나. 들어오면 user를 반환해라
...
//        when  (실행)
        Long foundId = userService.login("sdfsd", "sdfgdsfdsf");
//        then  (검증)
        assertThat(foundId).isNotNull().isEqualTo(1L);
    }
    @Test
    @DisplayName("로그인-예외확인 테스트")
    void loginException() {
//        given (준비)
