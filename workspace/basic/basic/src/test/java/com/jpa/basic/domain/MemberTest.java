...
//    단, AutoWried어노테이션이 아닌 @PersistenceContext라는 entityManager 전용 어노테이션을 사용한다.
    ...
    void memberTest(){
        ...
        member.setAge(22);
        member.setEmail("aaa@naber.com");
        member.setMemberGender(MemberGender.MALE);
        member.setPassword("1234");
        member.setName("김철수");

//        엔티티매니저는 엔티티를 넘겨받으면 영속성 켄텍스트 내부의[1차 캐시]라는 곳에 저장한다.
//        그리고 엔티티매니저는 쿼리도 생성을 해주는데 쿼리를 생성하게되면 영속성 컨텍스트 내부의[쓰기 지연 SQL 저장소]라는 공간에
//        SQL을 저장한다. 엔티티매니저에게 엔티티를 넘겨주고 INSERT쿼리를 생성하는 메소드가 persist()이다.
        entityManager.persist(member);

//        엔티티 매니저에게 엔티티를 넘겨주어도 DB에 저장한것이 아니다. -> 영속성 컨텍스트에 저장한 것이다.
//        여기서 쿼리를 날리고 싶다면 flush를 해줘야한다.
//        flush를 하면 DB에 해당 쿼리를 반영하고, commit을 해야 반영 내용을 확정짓는다.
//        그러므로 JPA를 사용하는 경우 항상 트랜잭션이 필요하다.
    }
