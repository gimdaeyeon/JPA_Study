...
//    생일이 특정 날짜보다 이전인 회원 조회
    List<User> findByBirthBefore(LocalDate birth);
...
//    이름에 특정 글자가 포함되고 핸드폰에 특정 글자가 포함되는 회원들을 id 내림차순으로 정렬하여 조회
    List<User> findByNameContainingAndPhoneContainingOrderByIdDesc(String name,String phone);
...
//   birth가 null이 아니고 id가 특정 범위안에 속하는 회원 모두 조회
    List<User> findByBirthNotNullAndIdBetween(Long id, Long id2);
