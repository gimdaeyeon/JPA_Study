...
//    쿼리 메소드를 사용하면 이름을 기반으로 쿼리르 쉽게 만들 수 있다는 장점이 있으나
//    다음과 같은 단점도 존재한다.
//    1. 조건이 많아지면 메소드의 이름이 너무 길어져서 가독성이 떨아진다.
...
//    jpal 에서도 is null/ is not null을 지원한다.
...
//    in 을 사용할 수 있다. (매개변수를 Collection<T>타입/ 자식나입 List,Set도 사용 가능)
...
//    dto로 반환을 받을 수 있다.
    @Query("select new com.jpa.data01.domain.dto.BookDto(b.id,b.category,b.name,b.price,b.releaseDate) from Book b where b.id = :id")
    BookDto findDtoById(@Param("id")Long bookId);

    @Query("select new com.jpa.data01.domain.dto.BookDto(b.id,b.category,b.name,b.price,b.releaseDate) from Book b where b.price > :price")
    List<BookDto> findDtoByPrice(@Param("price")Integer price);

//    Map으로 반환을 받을 수 있다.
//    map을 사용하는 경우 별칭이 key로 설정된다.(설정하지 안을 경우 0,1..이런식으로 키가 만들어진다.)
...
//    Map으로 반환할 때 여러 행을 반환해야한다면 List로 감싼다.
...
//    오라클에서 지원하는 형변환 함수 TO_CHAR(SYSDATE,'YYYY')는 방언이기 때문데 별도의 등록이 필요하다.
//    하이버네이트는 DB별 기본적인 함수들을 등록해놨기 때문에 바로 사용이 가능하다.
//    사용할 때는 function('함수명',인자1,인자2,인자3....) 으로 사용한다.
//    @Query("select b from Book b where function('to_char',b.releaseDate,'YYYY') = :year ")

//    방언을 직접 호출하여 사용하는것도 가능하지만 DB벤더사가 변경되면 문제가 생길 수 있다.
//    @Query("select b from Book b where to_char(b.releaseDate,'YYYY') = :year")
